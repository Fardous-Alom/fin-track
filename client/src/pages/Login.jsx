  ...
  return (
    <section className="min-h-screen flex items-center justify-center bg-gray-100">
      <div className="w-full max-w-md bg-white rounded-lg shadow-lg flex flex-col justify-center items-center p-10 mx-auto">
        <h2 className="text-gray-900 text-2xl mb-4 font-semibold title-font">Login</h2>
        ...
        <form className="w-full flex flex-col gap-5" onSubmit={handleSubmit}>
          <div className="flex flex-col">
            ...
            <input type="text" id="email" name="email" className="w-full bg-white rounded border border-gray-300 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-2 px-4 leading-8 transition-colors duration-200 ease-in-out" required onChange={e => setEmail(e.target.value)}/>
          </div>
          ...
          <button className="text-white bg-indigo-500 border-0 py-2 px-6 focus:outline-none hover:bg-indigo-600 rounded text-lg w-full" type='submit'>Login</button>
        </form>
        <p className="text-xs text-gray-500 mt-5 text-center">Don't have an account? <a href="/register" className="text-indigo-500">Register</a></p>
      </div>
    </section>
  )
}
